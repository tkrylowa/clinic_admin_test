...
public List<SpecializationGetInactiveDto> getSpecializations(boolean isActive) {
	...
	spec.forEach(s -> {
		int numberInactiveDocs = specializationDoctorRepository.countByIsActiveEqualsAndSpecializationsEquals(false, s.getId());
		int numberActiveDocs = specializationDoctorRepository.countByIsActiveEqualsAndSpecializationsEquals(true, s.getId());
